...
function FormularioBusca({tipo}: FormBuscaProps){
    ...
    const [resultado, setResultado] = useState<ICliente | IProduto | IServico | null>(null)
    
    function handleIdentificadorChange(event: React.ChangeEvent<HTMLInputElement>){
    ...
    function simulateSearch(identificador: string, tipo: string) {
        if (tipo === "cliente" && identificador === "12345678900") {
          return {
            nome: "João da Silva",
            cpf: "12345678900",
            rg: "1234567",
          ...
          <div className="row">
            <div className="input-field col s6" >
              ...
              <label htmlFor="identificador">
                {tipo === "cliente" ? "Digite o CPF do Cliente" : "Digite o ID"}
              </label>
            </div>
          </div>
          <div className="col s12">
            <Button type="button" variant="danger" onClick={handleSearch} name="action">
              Buscar
            </Button>
          </div>
        </form> 
